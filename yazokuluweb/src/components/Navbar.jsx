...
const Navbar = () => {
  ...
  return (
    <header className="bg-[#2D9596] shadow-md sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          {/* Logo */}
          <Link
            to="/home"
            className="flex items-center gap-2 no-underline hover:underline decoration-[#00224D] decoration-2 underline-offset-4"
          >
            <GraduationCap className="w-9 h-9 p-2 bg-[#143D60] text-white rounded-full" />
              ...
              <Link
                ...
                className={`px-4 py-2 rounded-lg font-medium transition text-sm ${
                  location.pathname === item.path
                    ...
                    : "text-white hover:bg-[#00224D] hover:text-white"
                }`}
              >
                {item.label}
              </Link>
